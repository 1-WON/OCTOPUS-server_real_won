...
class UserGrid extends Component {
  ...
  // 사용자관리 페이지에 data 띄워줄때 url과 userinfo에 컬럼 데이터를 담아주고있음.
  fetchData = () => {
    axios({
      method: 'POST',
      url: '/v1/user/list',
      data: {},
        ...
        (response) => {
          ...
          // 위 const userlist 를 선언해주고 setState에 담음.
  ...
  toggleRegisterUser = (isUpdate = true) => {
    ...
    if (!isUpdate) {
      this.setState({isRegisterUserModal: !this.state.isRegisterUserModal, userInfo: {email: "",status:1, roleIdList: []}})
    } else {
